...
test('Hand-drawn BV and CV screens keep separate navigation and exact controls',async t=>{
  ...
  click('[data-role="business"]');await until(()=>d.querySelector('#google-login'));click('#google-login');await until(()=>d.querySelector('#new-business'));
  ...
  w.testApi.navigate('/business/home');await until(()=>d.querySelector('.draw-chart'));assert.match(d.body.textContent,/Scan QR/);assert.match(d.body.textContent,/Invitații/);assert.equal(d.querySelectorAll('.bottom-nav__item').length,4);
  ...
  click('.draw-fab');await until(()=>d.querySelector('#event-add'));assert.ok(d.querySelector('textarea[name="name"]'));assert.equal(d.querySelectorAll('[name="duration"] option').length,11);assert.equal(d.querySelectorAll('.weekday-tabs input').length,7);
  ...
  w.testApi.navigate('/profile');await until(()=>d.querySelector('[data-account-plan="small"]'));assert.ok(d.querySelector('[data-account-plan="large"]'));assert.match(d.body.textContent,/Terms/);assert.match(d.body.textContent,/Privacy policy/);
