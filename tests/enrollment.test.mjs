...
test('Verified enrollment and universal developer access', async t => {
  ...
  await t.test('customer identity is stored once; booking approval and Romanian notification jobs are durable',async()=>{
    ...
    const statusJobs=(await db.query("select title,body,target_route,type from public.notification_jobs where booking_id=$1 and kind='status_update'",[bookingId])).rows;
    assert.equal(statusJobs.length,1);
    assert.equal(statusJobs[0].title,'Programare confirmată');
    assert.match(statusJobs[0].body,/^Salon Test · \d{2}\.\d{2}\.\d{4} \d{2}:\d{2} · Serviciu: Calendar principal$/);
    ...
    assert.equal((await db.query("select count(*)::int count from public.notification_jobs where booking_id=$1 and kind='reminder'",[rejectedId])).rows[0].count,0);
    const rejectedStatusJob=(await db.query("select title,body,target_route,type from public.notification_jobs where booking_id=$1 and kind='status_update'",[rejectedId])).rows[0];
    assert.equal(rejectedStatusJob.title,'Programare respinsă');
    assert.match(rejectedStatusJob.body,/^Salon Test · \d{2}\.\d{2}\.\d{4} \d{2}:\d{2} · Serviciu: Calendar principal$/);
    assert.equal(rejectedStatusJob.target_route,'/customer/notifications');
    assert.equal(rejectedStatusJob.type,'client');
