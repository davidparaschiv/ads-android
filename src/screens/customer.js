...
export async function companyScreen(root){
  ...
  root.innerHTML=page({title:'Fă programare',nav:'customer',active:'search',content:`<form class="draw-form" id="booking-request"><label>Alege serviciu<select name="service" id="service-select"><option value=""></option>${services.map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join('')}</select></label><label>Ore disponibile<select name="slot" id="slot-select" disabled></select></label><label>Durata<input id="service-duration" readonly></label><button class="draw-button" disabled>Solicită</button></form>`});
  ...
  serviceSelect?.addEventListener('change',async()=>{const service=services.find(item=>item.id===serviceSelect.value);if(!service||!slotSelect||!duration||!submit)return;duration.value=`${service.duration} min`;slotSelect.disabled=true;submit.disabled=true;slotSelect.innerHTML='';try{const slots=await availableServiceSlots(business.id,service.resourceId,service.id);slotSelect.innerHTML=slots.map(item=>`<option value="${escapeHtml(item.start_at)}">${slotLabel(item.start_at)}</option>`).join('');slotSelect.disabled=!slots.length;submit.disabled=!slots.length;}catch(error){toast(root,errorMessageForUser(error),'error');}});
  root.querySelector('#booking-request')?.addEventListener('submit',async event=>{event.preventDefault();const form=/** @type {HTMLFormElement} */(event.currentTarget);const service=services.find(item=>item.id===String(new FormData(form).get('service')));if(!service)return;const button=/** @type {HTMLButtonElement} */(form.querySelector('button'));try{loadingButton(button,true);await createBooking({businessId:business.id,eventTypeId:service.id,resourceId:service.resourceId,startAt:String(new FormData(form).get('slot')),customerName:store.get().user?.name||'',reminderMinutes:60});navigate('/customer/bookings');}catch(error){loadingButton(button,false);toast(root,errorMessageForUser(error),'error');}});
}
...
export async function customerBookingsScreen(root){
  ...
  root.innerHTML=page({title:'Progr. mele',nav:'customer',active:'bookings',content:`<section class="draw-section"><div class="draw-list">${bookings.map(item=>`<article class="customer-booking"><span><strong>${escapeHtml(item.business)}</strong><b>${escapeHtml(item.service)}</b><small>${escapeHtml(item.time)}, ${durationLabel(item.duration)}, ${formatDate(item.date)}</small><small class="booking-status booking-status--${escapeHtml(item.status)}"><i aria-hidden="true"></i>${escapeHtml(bookingStatusLabel(item.status))}</small></span><button data-booking-qr="${escapeHtml(item.id)}" aria-label="QR programare">${icon('qr')}</button></article>`).join('')}</div></section>`});
  bindBack(root);root.querySelectorAll('[data-booking-qr]').forEach(button=>button.addEventListener('click',async()=>{const id=button.getAttribute('data-booking-qr')||'';await store.set({selectedBookingId:id});navigate(`/customer/booking-qr?booking=${encodeURIComponent(id)}`);}));
}
...
function bookingStatusLabel(status){return ({pending:'În așteptare',confirmed:'Aprobat',rejected:'Respins',cancelled:'Anulat',completed:'Finalizat',no_show:'Absent'})[status]||'Stare necunoscută';}
