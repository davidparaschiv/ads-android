...
export async function configureBilling(userId) {
  ...
  if (configuredFor) await Purchases.logIn({ appUserID: userId });
  else await Purchases.configure({ apiKey: config.revenueCat.googleApiKey, appUserID: userId });
...
export async function purchasePlan(planId, userId) {
  ...
  await Purchases.purchasePackage({ aPackage: selected, ...(oldProductIdentifier ? {
